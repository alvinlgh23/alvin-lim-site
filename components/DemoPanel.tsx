  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
      <div className="rounded-lg border border-white/10 bg-white/[0.035] p-5">
        <p className="text-xs uppercase tracking-[0.24em] text-ember/80">Demo Panel</p>
        ...
        <div className="mt-6 space-y-4">
          {isValuationDemo ? (
            <>
              ...
              <select
                className="w-full rounded-md border border-white/12 bg-ink px-4 py-3 text-bone outline-none transition focus:border-ember"
                id="market-mode"
                onChange={(event) => {
              ...
              {activeMarketMode.needsInput ? (
                <>
                  ...
                  <input
                    className="w-full rounded-md border border-white/12 bg-ink px-4 py-3 text-bone outline-none transition placeholder:text-bone/35 focus:border-ember"
                    id="market-input"
                    onChange={(event) =>
              ...
              <input
                className="w-full rounded-md border border-white/8 bg-black/25 px-4 py-3 font-mono text-sm text-bone/72 outline-none"
                id="market-command"
                readOnly
                value={command}
              />
          ...
          <button
            className="w-full rounded-md bg-bone px-4 py-3 font-medium text-ink transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-60"
            disabled={isLoading}
            onClick={generateOutput}
            type="button"
          >
            {isLoading ? "Running Valuation-model..." : isValuationDemo ? "Generate Output" : "Generate Sample Output"}
          </button>
        </div>
        ...
        {isValuationDemo && activeMarketMode.needsInput ? (
          <p className="mt-3 text-xs leading-5 text-bone/42">
            {activeMarketMode.needsInput === "sector"
      ...
      <div className="rounded-lg border border-ember/20 bg-[#15130f] p-5 shadow-glow">
        <p className="text-xs uppercase tracking-[0.24em] text-ember/80">Output Panel</p>
        <h2 className="mt-3 text-2xl font-semibold text-bone">{demo.heading}</h2>
        {isValuationDemo ? (
          <>
            ...
            <pre className="mt-5 max-h-[620px] overflow-auto whitespace-pre-wrap rounded-md border border-white/8 bg-black/25 p-4 font-mono text-sm leading-6 text-bone/78">
          ...
          <>
            ...
            <div className="mt-5 space-y-4 font-mono text-sm">
              {demo.blocks.map((block) => (
                <section className="rounded-md border border-white/8 bg-black/25 p-4 text-bone/78" key={block.title}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <h3 className="whitespace-pre-line text-sm font-semibold leading-6 text-bone">{block.title}</h3>
                    {block.badge ? <span className="rounded-full border border-ember/30 px-2 py-1 text-[11px] text-ember">{block.badge}</span> : null}
                  </div>
